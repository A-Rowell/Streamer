...
void FGstTexture::ReleaseSample(IGstSample *Sample)
{
    ...
    {
        if (m_AppSink)
            m_AppSink->ReleaseSample(Sample);
        else
            Sample->Destroy();
    }
}
...
void FGstTexture::TickGameThread()
{
    ...
    if (Sample)
    {
        ...
        if (m_TextureObject)
        {
            ...
            ([Context, Sample](FRHICommandListImmediate &RHICmdList) {
...
void FGstTexture::Resize(IGstSample *Sample)
{
    ...
    if (m_GstFormat != SampleFormat || m_Width != Sample->GetWidth() || m_Height != Sample->GetHeight())
    {
        Release();
